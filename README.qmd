...
We developed two Python scripts ([`fetch_metadata_demographics.py`](https://github.com/matteoschmelzer/wobblyFriends/blob/main/transcripts/Fetch%20scripts/fetch_metadata_demographics.py) and [`fetch_textgrid_transcripts.py`](https://github.com/matteoschmelzer/wobblyFriends/blob/main/transcripts/Fetch%20scripts/fetch_textgrid_transcripts.py) to scrape the transcript data from the [Balance Corpus Explorer](https://thebalancecorpus.warwick.ac.uk/explore#). This provided us with the OpenAI Whisper transcripts that had been proof-checked by the authors. We decided to go with this approach instead of running Whisper locally because it saved time and provided us with higher quality transcripts for 10 dyads (only three dyads were provided in the [MDIG2026 Summer School GitHub repository](https://github.com/WimPouw/TilburgMultiscaleSummerschool2026)). This increase in data will give us more statistical power to test the hypotheses below. Note: the 10 dyads featured on the Balance Corpus Explorer explicitly gave consent for their data to be shared publicly.
